...
export JEST_JUNIT_OUTPUT_FILE="$XML_OUTPUT_FILE"
export JS_BINARY__BINDIR="bazel-out/darwin_arm64-fastbuild/bin"
export JS_BINARY__COMPILATION_MODE="fastbuild"
export JS_BINARY__TARGET_CPU="darwin_arm64"
export JS_BINARY__BUILD_FILE_PATH="common/BUILD.bazel"
export JS_BINARY__PACKAGE="common"
export JS_BINARY__TARGET_NAME="test"
export JS_BINARY__TARGET="//common:test"
...
export JS_BINARY__LOG_PREFIX="aspect_rules_jest[jest_test]"
...
To disable this validation you can set allow_execroot_entry_point_with_no_copy_data_to_bin to True in js_run_binary"
    ...
    entry_point="node_modules/.aspect_rules_js/jest-cli@29.7.0_at_types_node_18.14.0/node_modules/jest-cli/bin/jest.js"
    if [[ "$entry_point" == ../* ]]; then
        entry_point="external/${entry_point:3}"
    fi
    entry_point="$JS_BINARY__EXECROOT/$BAZEL_BINDIR/$entry_point"
else
    entry_point="$JS_BINARY__RUNFILES/_main/node_modules/.aspect_rules_js/jest-cli@29.7.0_at_types_node_18.14.0/node_modules/jest-cli/bin/jest.js"
fi
...
node="$(_normalize_path "_main/../rules_nodejs~~node~nodejs_darwin_arm64/bin/nodejs/bin/node")"
if [ "${node:0:1}" = "/" ]; then
    ...
    export JS_BINARY__NODE_BINARY="$JS_BINARY__RUNFILES/_main/../rules_nodejs~~node~nodejs_darwin_arm64/bin/nodejs/bin/node"
    if [ ! -f "$JS_BINARY__NODE_BINARY" ]; then
...
export JS_BINARY__NODE_WRAPPER="$JS_BINARY__RUNFILES/_main/common/test_node_bin/node"
...
export JS_BINARY__NODE_PATCHES="$JS_BINARY__RUNFILES/_main/../aspect_rules_js~/js/private/node-patches/register.js"
...
ARGS=()
ALL_ARGS=(--no-cache --ci --config 'common/test__jest.config.mjs' --colors --runInBand "$@")
